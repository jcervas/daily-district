...
## v2.13.49 — Center district-tile number labels (iOS Safari fix)

- District-tile number labels are now vertically centered via a measured `dy` offset instead of `dominant-baseline` (which iOS Safari ignores), so tile numbers no longer sit off-center on iOS. The correct-guess check uses the same measured offset.
...
- Compactness (area/perimeter/Reock/Polsby-Popper/adjacency) is now computed once, by a single shared `data/compactness.R`, used both to embed values in `districts.topojson` and to push them to `puzzles.census` — previously computed two different ways (`redistmetrics`/EPSG:5070 vs `sf`+`lwgeom`/EPSG:2163) that could disagree by a few percent on complex coastal districts (AK, HI, FL). Also fixed a tract→district crosswalk tie-break mismatch (alphabetical vs. insertion-order) that only affected a handful of exactly-50/50 split boundary tracts nationally.
- Rebuilt and pushed a full refresh of the District Profile data to production from the new pipeline: ACS demographics (`census`), current representatives (`reps`), hint cards (`clues`), 2020 population (`pop2020`), last-redrawn plan year (`plan-year`), shape metrics (`compactness`), non-English-speakers (`lang`), and percentile ranks (`derived`). `districts-core.topojson` and `state-acs.json` were also regenerated from the new pipeline.
