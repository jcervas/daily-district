...
DISPLAY = {s: b64(render("favicon-display.svg", f"d{s}.png", s))
...
def construction_svg(seam, ch, label):
    """The mark on its grid, with the channel dimensioned."""
    c = 24.0
    ox, oy = 30.0, 26.0
    ...
    for i in range(int(G) + 1):
        ...
        lines.append(f'<line x1="{ox:.0f}" y1="{y:.0f}" x2="{ox + G * c:.0f}" '
                     f'y2="{y:.0f}"/>')
    seam_pts = " ".join(f"{ox + x * c:.0f},{oy + y * c:.0f}" for x, y in seam)
    # dimension the channel across a vertical run near the top
    y0 = oy + 1.4 * c
    x1 = ox + (seam[0][0] - ch / 2) * c
    x2 = ox + (seam[0][0] + ch / 2) * c
    dim = (f'<line x1="{x1:.0f}" y1="{y0:.0f}" x2="{x2:.0f}" y2="{y0:.0f}"/>'
           f'<line x1="{x1:.0f}" y1="{y0 - 6:.0f}" x2="{x1:.0f}" y2="{y0 + 6:.0f}"/>'
           f'<line x1="{x2:.0f}" y1="{y0 - 6:.0f}" x2="{x2:.0f}" y2="{y0 + 6:.0f}"/>'
           f'<text x="{(x1 + x2) / 2:.0f}" y="{y0 - 13:.0f}" text-anchor="middle">'
           f'{ch}</text>')
  ...
  <polyline class="seam" points="{seam_pts}"/>
  <g class="dim">{dim}</g>
</svg>'''
...
<style>
  ...
  .cons .fill {{ fill:var(--red); fill-opacity:.15; stroke:var(--red);
                 stroke-width:2; }}
  .cons .seam {{ fill:none; stroke:var(--ink); stroke-width:1.6;
