...
// Full-screen loader (same spinning CSS globe as the welcome screen) shown while a heavy
// build runs with no other UI up — e.g. an archive fetch + map build. Pure CSS, so it
// keeps spinning through the synchronous build (no canvas/main-thread freeze).
...
async function startServerArchive(date, num, label) {
  showBuildLoader();   // CSS loader spins through the whole fetch + build (no freeze)
  let data;
