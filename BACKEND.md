...
- `oneoff_events(slug PK, district_id, state, title, clues, census, active, created_at)` —
  hand-seeded "special edition" rows for the one-off game at `/oneoff.html`, where every
...
Backs `/oneoff.html` — the one-off "special edition" mode where every player plays the
...
stays on (real traffic, not a throwaway round). `oneoff.html` is **generated** from
`index.html` by `build-oneoff.mjs`, the same way `demo.html` is generated by
